...
for folder in "$@"; do
	...
	if [ -v databases[$folder] ]; then
		...
		for table_data in $tables; do
			...
			file=/store/bphp/orm/$folder/$model_name"Model.php"
			...
			echo " */" >> $file
			echo "" >> $file
			echo "class "$model_name"Model" >> $file
			echo "{" >> $file
			echo -e "\tuse ORM;" >> $file
			echo "" >> $file
			...
			for key_data in $keys; do
				...
				if [ $column_position -eq 3 ]; then
					...
					echo "" >> $file
					echo -e "\tpublic function $rtable_model()" >> $file
					echo -e "\t{" >> $file
					echo -e "\t\t${rtable_model}Model::flush();" >> $file
					echo -e "\t\treturn ${rtable_model}Model::where(\"$column_name\", \$this->$referenced_column_name);" >> $file
					echo -e "\t}" >> $file
					column_position=0
				fi
			done
